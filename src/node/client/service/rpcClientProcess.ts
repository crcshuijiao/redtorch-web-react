...
class RpcClientProcess {
        ...
        switch (rpcId) {
            ...
            case RpcId.ORDER_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcOrderRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.TRADE_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcTradeRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.POSITION_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcPositionRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.ACCOUNT_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcAccountRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.CONTRACT_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcContractRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.TICK_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcTickRtn.commonRtn, sourceNodeId);
            ...
            case RpcId.NOTICE_RTN: {
                try {
                    ...
                    this.checkCommonRtn(rpcNoticeRtn.commonRtn, sourceNodeId);
    ...
    checkCommonRtn(commonRtn: xyz.redtorch.pb.ICommonRtnField | null | undefined, sourceNodeId: number) {
        if (!commonRtn) {
            console.error("参数commonRtn缺失");
            throw new Error("参数commonRtn缺失");
        }

        if (sourceNodeId !== commonRtn.sourceNodeId) {
            throw new Error("不合法的来源节点ID:" + commonRtn.sourceNodeId);
        }

        const nodeId = authenticationStore.nodeId
        if (commonRtn.targetNodeId !== nodeId) {
            throw new Error("目标节点ID不为" + nodeId + "!");
        }
    ...
    checkCommonRsp(commonRsp: xyz.redtorch.pb.ICommonRspField | null | undefined, sourceNodeId: number, reqId: string) {
        ...
        if (sourceNodeId !== commonRsp.sourceNodeId) {
            throw new Error("不合法的来源节点ID:" + commonRsp.sourceNodeId);
        }

        const nodeId = authenticationStore.nodeId
        if (commonRsp.targetNodeId !== nodeId) {
            throw new Error("目标节点ID不为" + nodeId + "!");
        }
